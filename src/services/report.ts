...
function summarizeMetric(values: Array<number | null>): MetricSummary {
  ...
  return {
    ...
    percentile95: roundToSingle(percentile(sortedValues, 0.95)),
...
function calculatePerformanceScore(data: PerfDataPoint[]): PerformanceScoreSummary {
  ...
  const cpuScore = cpuStats.sampleCount > 0
    ? interpolateScore(cpuStats.percentile95, [
      { value: 5, score: 100 },
      { value: 10, score: 90 },
      { value: 15, score: 80 },
      { value: 25, score: 65 },
  ...
  const ioScore = ioStats.sampleCount > 0
    ? interpolateScore(ioStats.percentile95, [
      { value: 128, score: 100 },
      { value: 256, score: 90 },
      { value: 512, score: 75 },
      { value: 1024, score: 55 },
...
export async function savePerformanceReport({
  ...
  drawMetricCard(
    ...
    `95分位 ${formatMetricValue(scoreSummary.cpuStats.percentile95, '%')}`,
  ...
  drawMetricCard(
    ...
    `95分位 ${formatMetricValue(scoreSummary.memoryStats.percentile95, 'MB')}`,
  );
  drawMetricCard(
    detailStartX + (detailCardWidth + scoreCardGap) * 2,
    'I/O 得分',
    scoreSummary.ioScore,
    `95分位 ${formatMetricValue(scoreSummary.ioStats.percentile95, 'KB/s')}`,
  ...
  drawMetricCard(
    ...
    `CPU 尖峰 ${formatRatio(scoreSummary.cpuSpikeRatio)}`,
    `I/O 尖峰 ${formatRatio(scoreSummary.ioSpikeRatio)}`,
  );
  ...
  const filePath = await save({
    title: '保存性能报告',
    defaultPath: `ACE-Lite_性能报告_${timestamp}.png`,
    filters: [
      {
        name: 'PNG 图片',
        extensions: ['png'],
